import { Product, ColorWithName } from "@/types";

const colorMap: Record<string, string> = {
  Verde: "#8bd741",
  Azul: "#274c9a",
  Rojo: "#e23c3c",
  Amarillo: "#ffec3d",
  Blanco: "#ffffff",
  Negro: "#121212",
  Gris: "#808080",
  Rosa: "#ff69b4",
};

const createColor = (name: string): ColorWithName => ({
  code: colorMap[name] || "#000000",
  name,
});

export const allProducts: Product[] = [
  {
    id: "1",
    name: "Sapito de Escritorio",
    price: 25.99,
    image: [
      "https://images.unsplash.com/photo-1615789591457-74a63395c990?w=400&h=300&fit=crop",
    ],
    description:
      "Figura articulada de sapito ideal para decorar escritorios y estanterías.",
    alt: "Sapito de escritorio en 3D",
    plasticType: "PLA",
    printTime: "2-3 horas",
    availableColors: ["Verde", "Azul", "Rojo", "Amarillo"].map(createColor),
    stock: 12,
  },
  {
    id: "2",
    name: "Marco Decorativo",
    price: 18.5,
    image: [
      "https://images.unsplash.com/photo-1578662996442-48f60103fc96?w=400&h=300&fit=crop",
    ],
    description:
      "Marco minimalista perfecto para resaltar fotografías o ilustraciones pequeñas.",
    alt: "Marco decorativo impreso en 3D",
    plasticType: "PETG",
    printTime: "1-2 horas",
    availableColors: ["Blanco", "Negro", "Gris"].map(createColor),
    stock: 9,
  },
  {
    id: "3",
    name: "Portalápices Creativo",
    price: 15.75,
    image: [
      "https://images.unsplash.com/photo-1611522135882-37d84284fdbc?w=400&h=300&fit=crop",
    ],
    description:
      "Portalápices compacto con compartimentos para organizar útiles de escritorio.",
    alt: "Portalápices creativo en 3D",
    plasticType: "PLA",
    printTime: "1 hora",
    availableColors: ["Verde", "Azul", "Rosa"].map(createColor),
    stock: 15,
  },
  {
    id: "4",
    name: "Organizador de Escritorio",
    price: 32.0,
    image: [
      "https://images.unsplash.com/photo-1600880292203-757bb62b4baf?w=400&h=300&fit=crop",
    ],
    description:
      "Organizador modular con espacios para notas adhesivas, lapiceras y accesorios.",
    alt: "Organizador de escritorio en 3D",
    plasticType: "PLA+",
    printTime: "3-4 horas",
    availableColors: ["Blanco", "Negro", "Verde", "Azul"].map(createColor),
    stock: 6,
  },
  {
    id: "5",
    name: "Sapito Porta Llaves",
    price: 12.99,
    image: [
      "https://images.unsplash.com/photo-1615789591457-74a63395c990?w=400&h=300&fit=crop",
    ],
    description:
      "Soporte de pared con forma de sapito para colgar llaves y pequeños accesorios.",
    alt: "Sapito porta llaves en 3D",
    plasticType: "PLA",
    printTime: "1 hora",
    availableColors: ["Verde", "Azul"].map(createColor),
    stock: 20,
  },
  {
    id: "6",
    name: "Decoración de Pared",
    price: 22.5,
    image: [
      "https://images.unsplash.com/photo-1578662996442-48f60103fc96?w=400&h=300&fit=crop",
    ],
    description:
      "Panel decorativo liviano para darle personalidad a cualquier ambiente.",
    alt: "Decoración de pared impresa en 3D",
    plasticType: "PETG",
    printTime: "2 horas",
    availableColors: ["Blanco", "Negro"].map(createColor),
    stock: 8,
  },
  {
    id: "7",
    name: "Sapito Lámpara",
    price: 45.0,
    image: [
      "https://images.unsplash.com/photo-1611522135882-37d84284fdbc?w=400&h=300&fit=crop",
    ],
    description:
      "Lámpara nocturna con figura de sapito y luz cálida para habitaciones infantiles.",
    alt: "Sapito lámpara en 3D",
    plasticType: "PLA",
    printTime: "4-5 horas",
    availableColors: ["Verde", "Amarillo", "Azul"].map(createColor),
    stock: 4,
  },
  {
    id: "8",
    name: "Macetero Sapito",
    price: 28.75,
    image: [
      "https://images.unsplash.com/photo-1600880292203-757bb62b4baf?w=400&h=300&fit=crop",
    ],
    description:
      "Macetero con diseño de sapito ideal para plantas pequeñas o suculentas.",
    alt: "Macetero sapito en 3D",
    plasticType: "PETG",
    printTime: "3 horas",
    availableColors: ["Verde", "Blanco", "Rosa"].map(createColor),
    stock: 10,
  },
];
